...
export const Icon = ({ name, size = 20, stroke = 1.6, style, ...rest }) => {
  const paths = {
...
export const Button = ({ variant = 'primary', size = 'md', icon, iconAfter, children, style, ...rest }) => {
...
export const Avatar = ({ parent, size = 32, ring }) => (
  <div title={parent?.name} style={{
    width: size, height: size, borderRadius: 999,
    background: parent?.color || '#888', color: '#fff',
...
export const AvatarCluster = ({ parents, size = 28, max = 4 }) => {
...
export const Eyebrow = ({ children, onDark, accent, style }) => (
  <div className={"sg-eyebrow" + (onDark ? ' on-dark' : '')} style={{ color: accent ? 'var(--sg-accent)' : undefined, ...style }}>
    {children}
  </div>
);

export const Drawer = ({ open, onClose, children, width = 480 }) => {
  useEffect(() => {
...
export const Wordmark = ({ size = 22, color, accent }) => (
  <span style={{
...
export const Spinner = () => (
  <div style={{
    ...
    border: '2.5px solid var(--sg-ink-10)',
    borderTopColor: 'var(--sg-accent)',
    animation: 'sg-spin 0.7s linear infinite',
  }}/>
);
// inject spin keyframe once
if (typeof document !== 'undefined' && !document.getElementById('sg-spin-kf')) {
  const s = document.createElement('style');
  s.id = 'sg-spin-kf';
  s.textContent = '@keyframes sg-spin { to { transform: rotate(360deg); } }';
