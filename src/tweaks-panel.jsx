// SummerGrid v2 — design tweaks panel (dev-only floating controls)
...
export function TweaksPanel({ title = 'Tweaks', children }) {
  ...
  // Keyboard shortcut: Ctrl+Shift+T opens panel
  useEffect(() => {
    const onKey = e => {
  ...
  if (!open) {
    return (
      <button onClick={() => setOpen(true)} style={{
        ...
        background: 'rgba(41,38,27,0.85)', color: '#fff', cursor: 'pointer',
        fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center',
        backdropFilter: 'blur(8px)',
      }} title="Open tweaks (Ctrl+Shift+T)">⚙</button>
    );
  }
...
export function TweakColor({ label, value, options, onChange }) {
  if (!options || !options.length) {
    return (
      <div className="twk-row twk-row-h">
        <div className="twk-lbl"><span>{label}</span></div>
        <input type="color" style={{ width: 56, height: 22, border: '.5px solid rgba(0,0,0,.1)', borderRadius: 6, padding: 0, cursor: 'pointer', background: 'transparent', flexShrink: 0 }}
